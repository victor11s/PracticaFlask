...
class ModelCita():
    ...
    def crearCita(cls, db, cita):
        try:
            ...
            # Verificamos si hay alguna cita existente con la misma hora y médico
            sql_verificar_citas = """SELECT COUNT(*) FROM CitaInformacion
                                     WHERE idUsuarioMedico = '{}' AND fechacita = '{}' AND 
                                     (horaInicio <= '{}' AND horaFin >= '{}')""".format(
                cita.idUsuarioMedico, cita.fechaCita, cita.horaFin, cita.horaInicio)
